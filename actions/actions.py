...
class AutoLocationAction(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker,
            domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        slots = []
        location = str(tracker.get_slot('auto_latitude')) + ',' + str(
            tracker.get_slot('auto_longitude'))
        ...
        dispatcher.utter_message(text='Your latitude is ' + latitude +
...
class FindSoltAction(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker,
        ...
        output = ''
        try:
            intent = tracker.latest_message['intent'].get('name').split(
                "_", 2)[2]
            ...
            for flow in data[intent]['flow']:
                    ...
                    for getSlots in flow['getSlots']:
                        if (getSlots['type'] == 'Single'):
                            resp = myRequests(
                                ...
                                slots_dict[getSlots['parameterName']][0])
                            utter_text = determinalStatusCode(
                                resp, flow['intent'])
                            dispatcher.utter_message(text=utter_text)
                        else:
                            for slot_value in slots_dict[
                                    getSlots['parameterName']]:
                                resp = myRequests(tracker, flow['intent'],
                                                  getSlots['parameterName'],
                                                  slot_value)
                                utter_text = determinalStatusCode(
                                    resp, flow['intent'])
                ...
                try:
                    for responseToSlots in flow['responseToSlots']:
                        ...
                        emp_ids_list = [
                            match.value for match in jsonpath_expression.find(
                                json.loads(resp.text))
                        ]
                        slots_dict[
                            responseToSlots['parameterName']] = emp_ids_list
                        print(slots_dict)
                except Exception as e:
                    print(e)
        ...
        for parameters in data[intent]['parameters']:
            if (parameters['name'] != slot):
                if (parameters['in'] == 'path'):
                    ...
                    headers[parameters['name']] = tracker.get_slot(
                        parameters['name'])
            # Processing response to slot
                ...
                if (parameters['in'] == 'path'):
                    ...
                    headers[parameters['name']] = slot_value
    except KeyError:
...
def determinalStatusCode(resp, intent):
    ...
    if (resp.ok):
        ...
        json_string = json.dumps(status_message,
